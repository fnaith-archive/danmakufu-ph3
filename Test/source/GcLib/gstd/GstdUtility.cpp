...
{
	std::wstring wstr = ConvertUtf8ToWide(text); //UTF16
	std::string strShiftJIS = ConvertWideToMulti(wstr); //ShiftJIS
...
std::wstring StringUtility::ConvertUtf8ToWide(std::vector<char>& text)
{
	...
	std::wstring wstr = ConvertMultiToWide(str, CP_UTF8); //UTF16
	return wstr;
}
...
wchar_t Scanner::_NextChar()
{
	if (HasNext() == false)
	{
		...
		std::wstring target = StringUtility::Format(L"? -> \r\n%s...", source.c_str());
...
Token& Scanner::Next()
{
	...
	switch (ch)
	{
	case L'\0': type = Token::TK_EOF; break;//
	...
	case L'"':
	{
		...
		else
		{
			...
			std::wstring log = StringUtility::Format(L"Next:?(String?) -> %s", error.c_str());
			_RaiseError(log);
		}
...
std::wstring& Token::GetIdentifier()
{
	if (type_ != TK_ID)
	{
		throw gstd::wexception(L"Token::GetIdentifier:");
	}
	return element_;
}
std::wstring Token::GetString()
{
	if (type_ != TK_STRING)
	{
		throw gstd::wexception(L"Token::GetString:");
	}
...
int Token::GetInteger()
{
	if (type_ != TK_INT)
	{
		throw gstd::wexception(L"Token::GetInterger:");
	}
...
double Token::GetReal()
{
	...
	{
		throw gstd::wexception(L"Token::GetReal:");
	}
...
TextParser::Result TextParser::_ParseSum(int pos)
{
	...
	while (scan_->HasNext())
	{
		...
		{
			if (tRes.type_ == TYPE_BOOLEAN)_RaiseError(L"");
			res.pos_ = tRes.pos_;
...
TextParser::Result TextParser::_ParseProduct(int pos)
{
	...
	while (scan_->HasNext())
	{
		...
		if (tRes.type_ == TYPE_BOOLEAN)_RaiseError(L"1241");
...
TextParser::Result TextParser::_ParseTerm(int pos)
{
	...
	else
	{
		...
		else _RaiseError(StringUtility::Format(L"?:%s", tok.GetElement().c_str()));
	}

	if (bMinus)
	{
		if (res.type_ != TYPE_REAL)_RaiseError(L"???");
		res.valueReal_ = -res.valueReal_;
	}
	if (bNot)
	{
		if (res.type_ != TYPE_BOOLEAN)_RaiseError(L"1311");
...
TextParser::Result TextParser::_ParseIdentifer(int pos)
{
	...
	{
		_RaiseError(StringUtility::Format(L":%s", id.c_str()));
	}
...
{
	if (scan_ == NULL)_RaiseError(L"1354");
	scan_->SetPointerBegin();
	Result res = _ParseComparison(scan_->GetCurrentPointer());
	if (scan_->HasNext())_RaiseError(StringUtility::Format(L"?:%s", scan_->GetToken().GetElement().c_str()));
	return res;
}
double TextParser::GetReal()
{
	if (scan_ == NULL)_RaiseError(L"1362");
	scan_->SetPointerBegin();
