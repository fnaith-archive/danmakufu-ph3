...
//    class DxScriptObjectBase;
    ...
    //DxScriptObjectBase
    **********************************************************/
    public abstract class DxScriptObjectBase : System.IDisposable
    {
// C++ TO C# CONVERTER TODO TASK: C# has no concept of a 'friend' class:
//        friend DxScript;
// C++ TO C# CONVERTER TODO TASK: C# has no concept of a 'friend' class:
//        friend DxScriptObjectManager;
            protected int idObject_;
            protected int typeObject_;
            protected _int64 idScript_ = new _int64();
            protected DxScriptObjectManager manager_;
            protected double priRender_;
            protected bool bVisible_;
            protected bool bDeleted_;
            protected bool bActive_;

            protected SortedDictionary<string, gstd.value> mapObjectValue_ = new SortedDictionary<string, gstd.value>();
...
//            void SetRenderPriorityI(int pri);
            ...
            {
                return mapObjectValue_.ContainsKey(key);
            }
            public gstd.value GetObjectValue(string key)
            {
                return new gstd.value(mapObjectValue_[key]);
            }
            public void SetObjectValue(string key, gstd.value val)
            {
                mapObjectValue_[key].CopyFrom(val);
            }
            public void DeleteObjectValue(string key)
            {
                mapObjectValue_.Remove(key);
            }
    }
