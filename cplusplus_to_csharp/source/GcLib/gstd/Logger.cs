...
namespace gstd
{
    ...
    **********************************************************/
    ...
    {
            protected static Logger top_ = null;
            protected gstd.CriticalSection lock_ = new gstd.CriticalSection();
            protected LinkedList<ref_count_ptr<Logger>> listLogger_ = new LinkedList<ref_count_ptr<Logger>>(); // �q�̃��K
            protected virtual void _WriteChild(SYSTEMTIME time, string str)
            {
                _Write(time, str);
                LinkedList<ref_count_ptr<Logger>>.Enumerator itr = listLogger_.GetEnumerator();
                while (itr.MoveNext())
                {
                    itr.Current._Write(time, str);
                }
            }

            protected abstract void _Write(SYSTEMTIME time, string str);
            ...
            **********************************************************/
            ...
            public void AddLogger(ref_count_ptr<Logger> logger)
            {
                listLogger_.AddLast(logger);
            }
